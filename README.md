...
`@terrytyc` 指定使用本项目软件源，避免同名包被其他源替换。软件源和 APK 均有签名；导入上述公钥后，也可上传 Release 中的 APK 安装，无需 `--allow-untrusted`。保留配置升级固件时，请将 `/etc/apk/keys/terrytyc-adguardhome.pem` 加入 `/etc/sysupgrade.conf`，一并保留公钥。
