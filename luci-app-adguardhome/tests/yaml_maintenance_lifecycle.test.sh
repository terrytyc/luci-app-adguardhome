...
printf '%s\n' "$prerm" | grep -Fq "trap 'rollback_yaml_maintenance' 0"
if printf '%s\n' "$prerm" |
...
if printf '%s\n' "$prerm" | grep -Eq '/etc/init.d/AdGuardHome (enable|start)'; then
	printf 'failed uninstall must not restart a service APK will remove\n' >&2
	exit 1
fi
before "$prerm" "trap 'rollback_yaml_maintenance' 0" begin_yaml_maintenance
