...
`luci-app-adguardhome` 2.4.0-r9，兼容基线为 ImmortalWrt 25.12.1（APK）。
...
2.4.0-r9 的活动配置只使用一个 UCI 文件：`/etc/config/adguardhome`。运行时不创建或使用 `/etc/config/AdGuardHome`，也不存在第二个 `enabled` 或第二个 `work_dir`。
...
- `2.4.0-r1`、`2.4.0-r2`、`2.4.0-r3`、`2.4.0-r4`、`2.4.0-r5`、`2.4.0-r6`、`2.4.0-r7` 和 `2.4.0-r8` 均支持原位升级到 `2.4.0-r9`。升级保留当前 UCI、YAML、HTTPS 配置、运行数据和内存模式设置；升级脚本不迁移或删除 2.4 基线以前的历史配置项。
- 安装或升级完成、服务状态恢复后，插件会重启一次 `rpcd`，清除旧进程可能保留的 ucode 文件描述符并加载新模块。LuCI 会话可能中断，需要重新登录；建议通过 SSH 执行 APK 安装。正常“保存并应用”不重启 `rpcd`。
- 从 2.3 及更早版本或未知开发版升级不受支持；应先完整卸载旧 LuCI 插件，再安装 2.4.0-r9。官方 `adguardhome` 核心软件包无需卸载。
...
- 安装完成后只重启一次 `rpcd`，不增加常驻进程、计时器或运行依赖；日常监控、页面查询和 data 回写行为保持不变。
