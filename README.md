...
上游模式要求只有一个 dnsmasq 实例，保留条件转发；已有普通上游或匹配所有域名的转发规则时会拒绝接管。启用后添加 `127.0.0.1#<DNS 端口>` 并设置 `noresolv=1`。停用或离开此模式时，删除插件记录的上游、取消 `noresolv`，不改 `resolvfile`。其他模式切换也只撤销插件创建的接管项；无法确定归属时会报错，不猜测清理目标。
