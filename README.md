...
- “YAML 配置”：编辑、校验、保存并应用 `<work_dir>/AdGuardHome.yaml`。只有校验成功并安全落盘后才应用；失败不会留下半写入的活动配置。
...
- 没有既有 AdGuard Home 状态的洁净安装会创建上述唯一 UCI 布局，按样例默认启用服务，并在默认工作目录缺少 YAML 时安装默认模板；导入已存在的官方实例时会保留其启用和运行状态。
