...
- “运行日志”页把官方核心日志和插件协调器日志分开显示，均按时间倒序排列，最新记录在最上方
- “设置”页可修改 YAML 中唯一管理账号的用户名、密码或两者；支持非 `admin` 用户名，密码仍在浏览器内使用 cost 10 的 BCrypt 生成哈希，路由器不会接收密码明文
- 可选“从内存运行”：启动时只把持久化工作目录中的 `data` 复制到 RAM 挂载路径，并让官方核心在该副本中运行；`AdGuardHome.yaml` 始终从持久化工作目录读取和修改，不复制到内存；正常停止或重启时仍会执行完整回写
...
- 支持从正式版 `2.1.0-r1`、`2.2.0-r1`、`2.2.0-r2` 或 `2.3.0-r1` 原位升级到 `2.3.0-r2`；升级会保留当前 UCI、工作目录、YAML、HTTPS 配置与运行数据。2.2/2.3r1 的旧式内存模式会在升级边界先完整回写，再按 r2 的“持久 YAML + 内存 data”布局重建
...
从 `2.1.0-r1`、`2.2.0-r1`、`2.2.0-r2` 或 `2.3.0-r1` 可直接通过 APK 包管理器升级到 `2.3.0-r2`。升级会保留 YAML、UCI、DNS、运行数据和内存模式设置；工作目录切换从 2.3 起不再复制旧目录内容。从 1.x、2.0.x 或未知开发版升级不受支持，应先使用包管理器完整卸载（APK 系统建议使用 `apk del --purge`）旧 LuCI 插件再安装 2.3；官方 `adguardhome` 核心包无需卸载。卸载 2.3 时仍会恢复首次安装 2.1 前保存的官方状态。
